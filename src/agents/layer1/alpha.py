"""Alpha Agent - AI Hardware & Semiconductors Specialist."""
...
class AlphaAgent(Layer1Agent):
    """Alpha agent specializing in AI hardware and semiconductors."""

    # Default coverage universe for hardware
    DEFAULT_TICKERS = [
        "NVDA", "AMD", "INTC", "QCOM",  # GPU/Chips
        "MU", "MRVL", "AVGO", "AMAT",    # Memory/Equipment
        "TSM", "ASML", "LRCX", "KLAC",   # Foundry/EDA
        "ARM", "SNPS", "CDNS",           # IP/EDA
        "ANET", "SMCI",                  # Networking/Infrastructure
    ]
    ...
    async def analyze(
        ...
        """Analyze hardware/semiconductor companies.
        ...
        Returns:
            AgentOutput with top 5 hardware picks
        """
        ...
        return AgentOutput(
            ...
            layer=self.layer,
            picks=picks,
            reasoning=f"Analysis based on {len(data.get('companies', {}))} companies in hardware sector.",
            metadata={
    ...
    def _is_relevant_sector(self, company_data: Any) -> bool:
        ...
        if isinstance(company_data, str):
            ...
            relevant_terms = [
                "semiconductor", "hardware", "chip", "memory",
                "foundry", "gpu", "processor", "networking",
            ]
